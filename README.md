...
| 色差 tolerance | 1〜100 | 50 | 魔法の杖が同色とみなす RGB の最大差 |
| 境界膨張 | 1〜100 | 50 | 魔法の杖の結果を広げる量 |
...
魔法の杖の境界膨張は下限が 1 だが、この式のせいで**幅 10000px 未満の画像では 1 も 0 も 0px** なのだ（幅 1024px なら 10 未満がすべて 0px）。膨張なしは下限側でそのまま選べる。
...
ファイルが無い・JSON が壊れている場合はコード内のデフォルト値にフォールバックする。書かれていないキーだけデフォルトで補われるのだ。`brush_shape` に `"circle"` `"square"` 以外を書いた場合は `"circle"` に戻され、`brush_size` は 3〜50、`wand_tolerance` と `wand_dilate_scale` は 1〜100 にクランプされる（いずれも起動はする）。
